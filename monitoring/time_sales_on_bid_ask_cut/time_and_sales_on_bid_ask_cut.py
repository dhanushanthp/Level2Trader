...
class TimeSalesBidAsk:
    ...
    def data_generator(self, time, bid_price, bid_size, ask_price, ask_size, last_price, last_size):
        ...
        if time in self.time_accumulator_on_bid:
            ...
            if last_price in self.time_accumulator_on_bid[time]:
                if last_price == bid_price:
                    # Get the value dictionary by time and price and update the size
                    self.time_accumulator_on_bid[time][last_price] = self.time_accumulator_on_bid[time][
                ...
                if last_price == bid_price:
                    ...
                    self.time_accumulator_on_bid[time][last_price] = last_size
                else:
                    # Time exist but the price is not exist, create a element with price and size
                    self.time_accumulator_on_bid[time][last_price] = 0
        else:
            # New element creation with time, price and size
            # Create value dictionary with size
            value_dict = dict()
            if last_price == bid_price:
                value_dict[last_price] = last_size
            else:
                value_dict[last_price] = 0

            # Create time dictionary with value
            self.time_accumulator_on_bid[time] = value_dict

        """
        Creation on ASK PriceUtil
        """
        if time in self.time_accumulator_on_ask:
            # If time already exist in dictionary
            if last_price in self.time_accumulator_on_ask[time]:
                if last_price == ask_price:
                    # Get the value dictionary by time and price and update the size
                    self.time_accumulator_on_ask[time][last_price] = self.time_accumulator_on_ask[time][
                                                                            last_price] + last_size
                else:
                    # Get the value dictionary by time and price and update the size
                    self.time_accumulator_on_ask[time][last_price] = self.time_accumulator_on_ask[time][
                ...
                if last_price == ask_price:
                    # Time exist but the price is not exist, create a element with price and size
                    self.time_accumulator_on_ask[time][last_price] = last_size
                else:
                    # Time exist but the price is not exist, create a element with price and size
                    self.time_accumulator_on_ask[time][last_price] = 0
        else:
            # New element creation with time, price and size
            # Create value dictionary with size
            value_dict = dict()

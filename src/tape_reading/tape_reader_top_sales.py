...
class TapeReader:
    ...
    def generate_top_sales_overtime(self, tick_time: str, fraction_of_time: int = 10):
        """
        Generate fraction of time track top sales over a period of time
        :param tick_time:
        :param fraction_of_time
        :return:
        """

        tick_time_split = tick_time.split(":")
        time_by_fraction = ':'.join(tick_time_split[:-1]) + ':' + self.tu.round_time(int(tick_time_split[-1]), base=fraction_of_time)

        """
        Track the top sales by time. If time doesn't match to previous time then write the data to the file. The
        """
        if self.top_sales_previous_time is None:
            self.top_sales_previous_time = time_by_fraction

        # Write to a output file when the given time frame is not matched to previous time frame.
        # If the timeframe is 30s then the file will be write at 31st second
        if self.top_sales_previous_time != time_by_fraction:
            """
            ...
            # Limit the price to last n ticks (could be minutes, 10s of seconds or n's of seconds)
            ...
            # Set the current time to previous time once the file is written
            self.top_sales_previous_time = time_by_fraction
    ...
    def time_and_sales_histogram(self, global_price_limit, global_time_limit, last_ask_sizes, last_bid_sizes):
        ...
        for price in global_price_limit:
            ...
            # Clean 0 values
            ask_ratio = ask_ratio if ask_ratio != 0.0 else ''
            bid_ratio = bid_ratio if bid_ratio != 0.0 else ''
    ...
    def display_data(self, closest_price: float, bid_price: float, ask_price: float, source: str):
        ...
        for current_time in global_time_limit:
            ...
            for current_price in global_price_limit:
                ...
                if current_price in self.dict_last_size_on_bid[current_time] and current_price in self.dict_last_size_on_ask[current_time]:
                    ...
                    normal_transaction = color(self.pu.slot_convertor(last_bid_size), fore=clr_front_last_bid, back=clr_back_bid) + ' ➜ ' + color(
                        self.pu.slot_convertor(last_ask_size), fore=clr_front_last_ask, back=clr_back_ask)
        ...
        table_data.insert(0, global_time_limit + ['Price', concon_bids, concon_asks])
